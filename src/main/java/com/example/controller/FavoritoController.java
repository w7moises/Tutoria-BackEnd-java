...
public class FavoritoController {
    ...
    public ResponseEntity<?> createFavorito(@Valid @RequestBody Favorito favorito, BindingResult result){
        ...
        Optional<Docente> docenteRegistered = docenteService.findById(favorito.getDocente().getId());
        Optional<Padre> padreRegistered = padreService.findById(favorito.getDocente().getId());
        if (!docenteRegistered.isPresent() || !padreRegistered.isPresent()) return ResponseEntity.badRequest().build();
